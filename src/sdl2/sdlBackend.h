...
// its defaults (matching cglpSDL2.c's own CLI-parsed-then-apply ordering).
...
// possible) - matches cglpSDL2.c's own nodelay flag.
void sdlBackend_setVsync( bool enabled );
// Forces SDL2's built-in software renderer ("-s") instead of the default
// SDL_RENDERER_ACCELERATED request - see sdlBackend_init() for how this
// maps to SDL_CreateRenderer()'s own flags argument.
...
// way cglpSDL2.c's own makescreenshots pass does (one frame released, one
