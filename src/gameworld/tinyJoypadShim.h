...
// machineDependent.h - see obonoCoreShim.h's own identical comment on the
// same include for the full reasoning (this header uses `bool` too:
// isLeftPressed()/isFirePressed()/etc).
#include <stdbool.h>
...
// untouched. Ported unchanged (no dialect fixes needed at all - this shim
// has no arrays, strings, or structs) from the sibling tinyjoypad_vircon32
// build's own tinyJoypadShim.h.
// -----------------------------------------------------------------------------
