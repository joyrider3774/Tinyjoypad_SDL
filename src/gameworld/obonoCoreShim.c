...
// corrupting unrelated state. Bump this again if a future game's own
// board/sprite count exceeds it - there's no cost to headroom here beyond
// a few words of RAM. (Ported unchanged from the sibling tinyjoypad_vircon32
// build's own obonoCoreShim.c, where this was a real, found-and-fixed bug.)
#define SPRITES 20
#define STRINGS 10
...
SpriteT sprite[ SPRITES ];
StringT string[ STRINGS ];
int buttonState = 0;
int lastButtonState = 0;
int justPressedState = 0;
int wireBuffer[ WIDTH + 1 ];
...
// `tickWindow` is how many real frames pass between two calls to this
// function for the calling game (1 for an unthrottled 60fps game, 2 for a
// halved-rate 30fps one, 3 for NumberPlace's 20fps) - see
...
// strings, same as the Vircon32 build's own table. Functional bitmap data,
// not creative text.
int imgFont[ 63 ] = {
...
// note-by-note in the background, independent of the main loop. This
// backend exposes no interrupts/ISRs to games either, so the sequencer is
// an explicit per-frame poll instead: obonoCoreShimUpdateSound(), called
// once per frame from main.c exactly like md_updateAudio(), advances to
// the next note once the current one's frame countdown reaches zero.
int scorePos = -1;
...
{
    playingScore = NULL;
    md_playTone( (float)frequency, (float)duration / 1000.0f );
}
...
void obonoCoreShimAdvanceScore()
{
    ...
    md_playTone( (float)frequency, (float)durationMs / 1000.0f );
    scoreStopAtFrame = md_getFrameCounter() + (int)( ( (float)durationMs / 1000.0f ) * MD_FRAMES_PER_SECOND );
}
...
{
    if( playingScore != NULL && md_getFrameCounter() >= scoreStopAtFrame )
      obonoCoreShimAdvanceScore();
}
...
{
    // Self-assign to silence "unused parameter" warnings - matches the
    // Vircon32 build's own workaround for having no (void)param; idiom;
    // kept here too even though this compiler does support that idiom,
    // purely so this file stays a close diff against its sibling.
    signature = signature;
    address = address;
    pRecord = pRecord;
    size = size;
