...
// Ported unchanged from the sibling tinyjoypad_vircon32 build's own
// tinyJoypadShim.c - no dialect fixes needed anywhere in this file.
...
void InitTinyJoypad()
{
    ...
    // game started (see main.c) - nothing left to do here.
}
...
void Sound( int freq, int dur )
{
    ...
    // this platform's real audio oscillator (via md_playTone) instead of
    // bit-banged.
    int periodUs = 255 - freq;
    if( periodUs < 1 )
      periodUs = 1;

    float freqHz = 500000.0f / (float)periodUs;
    ...
    if( freq <= 0 )
    {
        ...
        md_playTone( 0.0f, durationSeconds );
        return;
    }
