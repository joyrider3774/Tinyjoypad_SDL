...
//
// Same driver lineage as every other Daniel-C-family game already in this
// project - no new shim needed: `isLeftPressed()`/`isRightPressed()`/
// `isUpPressed()`/`isFirePressed()`/`Sound()` from the existing
// `tinyJoypadShim` cover the whole input/audio surface, and
// `tfiBlitzSprite()`/`tfiSpeedBlitz()`/`tfiBlitzSpriteDir()` below are the
// same already-proven translation of `blitzSprite`/`SPEED_BLITZ`/the
// left-right-mirroring wrapper this project's `gameTinyBert.c`/
// `gameTinyMissile.c`/`gameJumpSlime.c` already carry under their own
// prefixes.
//
