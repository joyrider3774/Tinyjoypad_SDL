...
// pattern works in this codebase (maintain a real in-memory pixel
// buffer, stream it out via `md_drawColumn()` once per frame) - and
// critically, the room is redrawn completely from scratch every single
// real tick (`DrawRoomInProgress()` calls `MEGA82XX.clear()` then re-runs
// the whole vector command list every frame, not just once on room
// entry), so there's no need to persist buffer state *across* frames
// either - just rebuild it fresh each tick, exactly like every other
// game here already does with its own composed page buffer.
//
...
GitdPlayer gitdP;
GitdAcidDrop gitdAcid[6];
GitdBalloon gitdBalloon;
GitdTimeout gitdTMout;
GitdLevel gitdLvl;

int gitdVisibleKeyAndDoors[22];
int gitdVisibleSprite[30];
...
int gitdOldPic[888];   // 111 columns x 8 pages, matches upstream's own OldPic[111*8]
int gitdTransitStep;
int gitdTransitWait;
