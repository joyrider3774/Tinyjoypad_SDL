...
//   calls that would only cost real engine CPU budget for zero effect.
...
//   to-back) is a genuine burst - this engine's own audio channel has no
//   queue, so 4 same-tick calls would only ever be audible as the last one
//   - fixed proactively with a small frame-stepped sequencer, the same
//   treatment as every other multi-call burst already found in this
//   project. The level-up cue (`for(i=800;i>200;i-=200)beep(30,i)`, 3
//   calls) and the game-over/new-high sweeps are, byte-for-byte, the exact
//   same loop shapes already fixed for Stacker/Breakout's own identical
//   cues (same author/boilerplate lineage) - reused via the same derived
//   note tables rather than re-deriving them from scratch. The single
//   alien-hit beep needs no sequencing (only ever one call).
...
//   high score (`spaShowResetMsg`, a "-HIGH SCORE RESET-" banner); fire
...
int spaTextByte( int x, int y, int startX, int pageY, char* str )
{
    if( y != pageY ) return 0;
...
int spaAlienSprite1[8] = { 0x98, 0x5C, 0xB6, 0x5F, 0x5F, 0xB6, 0x5C, 0x98, };
int spaAlienSprite2[8] = { 0x30, 0x3E, 0xB3, 0x5D, 0x5D, 0xB3, 0x3E, 0x30, };
int spaMothershipSprite[8] = { 0x18, 0x38, 0x34, 0x34, 0x34, 0x34, 0x38, 0x18, };
