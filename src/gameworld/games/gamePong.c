...
int pongTextByte( int x, int y, int startX, int pageY, char* str )
{
    if( y != pageY ) return 0;
...
int pongTextByteLen( int x, int y, int startX, int pageY, char* str, int strLen )
{
    if( y != pageY ) return 0;
    if( x < startX ) return 0;
    int rel = x - startX;
...
void pongRenderFrame( int mode )
{
    ...
    for( y = 0; y < 8; y++ )
    {
        ...
        // per character), so `int* s = "text";` compiled there - in
        // standard C a literal is char[], and assigning/passing it to an
        // int* would silently reinterpret its bytes 4-at-a-time as
        // garbage ints instead of individual characters. Every text-byte
        // helper in this file (pongTextByte/pongTextByteLen) and this
        // variable were switched to char* together; strlen() also now
        // gets its actually-expected char* instead of a mismatched int*.
        char* attractStr = "";
        int attractStrLen = 0;
        int attractStartX = 0;
