...
//
// On-screen text is kept as `int[]` character-code arrays throughout
// (matching this project's own established convention - see e.g.
// gameOroboros.c's `orbTxtTitle`/`orbTextByteAt`), not converted to
// `char*`: the sibling Vircon32-dialect source's string literals are
// themselves plain `int[]` (one 32-bit word per character), so a bare
// `int* t = "BULLS & COWS";` assignment there is genuinely fine - in
// standard C the same line would silently misread a real `char[]`
// string literal's packed single-byte characters as 4-byte words. The
// 6 attract-screen string literals below (`tbcTxtTitle` etc) are ported
// as named `int[]` arrays of individual character constants instead,
// keeping `tbcFontByte()`/`tbcTextByteAt()` themselves unchanged.
// =============================================================================
...
void tbcComposeAnswerSquares( int page )
{
    ...
    if( !tbcGameOver )
    {
        ...
        int row1[7] = { 0xFF, 0x20, 0x20, 0x24, 0x20, 0x20, 0xFF };
        int row2[7] = { 0xFF, 0x08, 0x08, 0x49, 0x08, 0x08, 0xFF };
        int row3[7] = { 0xFF, 0x82, 0x82, 0x92, 0x82, 0x82, 0xFF };
        int* src = row3;
        if( page == 1 ) src = row1;
    ...
    if( page == 0 )
    {
        ...
        {
            int v[7] = { 0xC0, 0x40, 0x40, 0x40, 0x40, 0x40, 0xC0 };
...
void tbcComposeMenuText( int page )
{
    ...
    if( tbcSelectedRow < tbcGuess )
    {
        if( page == 2 )
        {
            int t[9] = { ' ','B','u','l','l','s',' ', 48 + tbcGuesses[ tbcSelectedRow ][ 4 ], ' ' };
            for( i = 0; i < 9; i++ ) text[ i ] = t[ i ];
        }
        else
        {
            int t[9] = { ' ','C','o','w','s',' ',' ', 48 + tbcGuesses[ tbcSelectedRow ][ 5 ], ' ' };
            for( i = 0; i < 9; i++ ) text[ i ] = t[ i ];
        }
    }
    else if( tbcGameOver )
    {
        if( page == 2 )
        {
            if( tbcGameWon ) { int t[9]={' ','Y','o','u',' ','W','o','n',' '}; for(i=0;i<9;i++) text[i]=t[i]; }
            else { int t[9]={' ','Y','o','u',' ','L','o','s','t'}; for(i=0;i<9;i++) text[i]=t[i]; }
        }
        else
        {
            int edge = ' ';
            ...
            int t[9] = { edge,'R','e','s','t','a','r','t',edge2 };
            for( i = 0; i < 9; i++ ) text[ i ] = t[ i ];
        }
    }
    else
    {
        ...
        else
        {
            ...
            int t[9] = { edge,'G','i','v','e',' ','U','p',edge2 };
            for( i = 0; i < 9; i++ ) text[ i ] = t[ i ];
        }
    }
...
// Attract-screen text, as int[] character-code arrays - see this file's
// own header comment on why these aren't char[]/char* string literals.
int tbcTxtTitle[12] = { 'B','U','L','L','S',' ','&',' ','C','O','W','S' };
int tbcTxtGuessFour[18] = { 'G','U','E','S','S',' ','F','O','U','R',' ','N','U','M','B','E','R','S' };
int tbcTxtBullsCorrect[17] = { 'B','U','L','L','S',' ','A','R','E',' ','C','O','R','R','E','C','T' };
int tbcTxtCowsWrong[20] = { 'C','O','W','S',' ','A','R','E',' ','W','R','O','N','G',' ','P','L','A','C','E' };
int tbcTxtByDatacute[11] = { 'B','Y',' ','D','A','T','A','C','U','T','E' };
int tbcTxtPressFire[10] = { 'P','R','E','S','S',' ','F','I','R','E' };
...
void tbcRenderAttract()
{
    ...
    for( page = 0; page < 8; page++ )
    {
        ...
        if( page == 0 )
        {
            for( col = 0; col < 128; col++ ) tbcPageBuffer[ col ] |= tbcTextByteAt( tbcTxtTitle, 12, 28, col );
        }
        if( page == 2 )
        {
            for( col = 0; col < 128; col++ ) tbcPageBuffer[ col ] |= tbcTextByteAt( tbcTxtGuessFour, 18, 10, col );
        }
        if( page == 3 )
        {
            for( col = 0; col < 128; col++ ) tbcPageBuffer[ col ] |= tbcTextByteAt( tbcTxtBullsCorrect, 17, 13, col );
        }
        if( page == 4 )
        {
            for( col = 0; col < 128; col++ ) tbcPageBuffer[ col ] |= tbcTextByteAt( tbcTxtCowsWrong, 20, 4, col );
        }
        if( page == 6 )
        {
            for( col = 0; col < 128; col++ ) tbcPageBuffer[ col ] |= tbcTextByteAt( tbcTxtByDatacute, 11, 34, col );
        }
        if( page == 7 )
        {
            for( col = 0; col < 128; col++ ) tbcPageBuffer[ col ] |= tbcTextByteAt( tbcTxtPressFire, 10, 34, col );
        }
