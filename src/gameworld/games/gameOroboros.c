...
int orbGrid[512]; // ORB_GRID_H * ORB_GRID_W, 1 = snake segment here
...
// On-screen text, as int[] character-code arrays. Upstream's Vircon32-dialect
// version passed C string literals directly to orbTextByteAt() (that
// dialect's string literals are plain int[], one word per character - see
// VIRCON32_C_DIALECT.md), which standard C's char[]-typed string literals
// can't do safely (a char* handed to an int* parameter would misread the
// packed single-byte characters as 4-byte words). Converted to named int[]
// arrays instead, matching this project's own established convention for
// on-screen text (see e.g. gameTinyInvaders.c's tinvTxtOneUp/tinvTxtLevel) -
// same characters, same order, same length as each original string literal.
int orbTxtTitle[15] = { 'O',' ','R',' ','O',' ','B',' ','O',' ','R',' ','O',' ','S' };
int orbTxtByWebboggles[17] = { 'b','y',' ','w','e','b','b','o','g','g','l','e','s','.','c','o','m' };
int orbTxtTweetWebboggles[17] = { 'T','w','e','e','t',' ','@','w','e','b','b','o','g','g','l','e','s' };
int orbTxtAttinyArcade[13] = { '#','A','t','t','i','n','y','A','r','c','a','d','e' };
int orbTxtGameOver[9] = { 'G','a','m','e',' ','O','v','e','r' };
int orbTxtScore[6] = { 's','c','o','r','e',':' };
int orbTxtTopScore[10] = { 't','o','p',' ','s','c','o','r','e',':' };

int orbScoreDigits[4];
int orbScoreDigitCount;
int orbTopScoreDigits[4];
...
int orbComputeByte( int x, int page )
{
    ...
    {
        if( page == 2 ) return orbTextByteAt( orbTxtTitle, 15, 16, x );
        if( page == 4 ) return orbTextByteAt( orbTxtByWebboggles, 17, 10, x );
        if( page == 6 ) return orbTextByteAt( orbTxtTweetWebboggles, 17, 12, x );
        if( page == 7 ) return orbTextByteAt( orbTxtAttinyArcade, 13, 22, x );
        return 0;
    }

    if( orbState == ORB_STATE_GAMEOVER )
    {
        if( page == 3 ) return orbTextByteAt( orbTxtGameOver, 9, 32, x );
        if( page == 5 )
        {
            int pixels = orbTextByteAt( orbTxtScore, 6, 32, x );
            pixels = pixels | orbTextByteAt( orbScoreDigits, orbScoreDigitCount, 70, x );
            return pixels;
        }
        if( page == 6 )
        {
            int pixels = orbTextByteAt( orbTxtTopScore, 10, 32, x );
