...
//   2-byte big-endian score at address 2/3, not 0/1, since this game
//   shares one combined cartridge's own EEPROM layout with UFO's own
//   addr 0/1) - the "hold fire ~2s to mute/unmute" gesture is kept
//   (in-memory flag); the combined-cartridge-specific "hold fire+up/
...
// character) - a standard C literal is char[], so an int* parameter fed
// literal strings (as every call site of this function does) would
// silently reinterpret their bytes 4-at-a-time as garbage ints.
int stkTextByte( int x, int y, int startX, int pageY, char* str )
{
    if( y != pageY ) return 0;
...
int stkMovingRow[16]; // upstream's row[0]
int stkTargetRow[16]; // upstream's row[1] (transient - only meaningful
                      // during the comparison at the moment of a lock)
int stkLockedRows[8][16]; // persistent per-screen-row locked pattern - see
...
int stkAttractStairStart[6] = { 14, 13, 13, 13, 12, 12 };
int stkAttractStairEnd[6]   = { 14, 14, 15, 15, 15, 15 }; // inclusive
