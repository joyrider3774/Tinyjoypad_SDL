...
int rddPlayerIdle1[5] =
{
60,219,194,219,60,
};

int rddPlayerIdle2[5] =
{
59,215,197,215,59,
};

int rddPlayerRight1[5] =
{
247,123,155,2,111,
};

int rddPlayerRight2[5] =
{
63,211,226,27,255,
};

int rddPlayerLeft1[5] =
{
111,2,155,123,247,
};

int rddPlayerLeft2[5] =
{
255,27,226,211,63,
};
...
int rddScoreDigits[4];
int rddScoreDigitCount;
int rddTopScoreDigits[4];
int rddTopScoreDigitCount;
int rddLiveScoreDigits[4];
...
// The following are plain int-array copies of the on-screen text used by
// rddComputeByte() below. Vircon32's own dialect allows a string literal
// to be passed anywhere an `int*` parameter is expected (its string
// literals are themselves int arrays, since the dialect has no `char`
// type at all) - standard, portable C has no such allowance (a string
// literal is a `const char[]`, not assignment-compatible with `int*`), so
// each literal originally passed straight to rddTextByteAt() is declared
// here as a real int array instead. Kept as `int` throughout (matching
// this game's own fully self-contained int-based font/text engine, not
// converted to `char`), and character constants (`'w'`, `'e'`, ...) are
// themselves already of type int in C, so no cast is needed anywhere.
int rddTextWebbogglesCom[14] = { 'w','e','b','b','o','g','g','l','e','s','.','c','o','m' };
int rddTextTweetWebboggles[17] = { 'T','w','e','e','t',' ','@','w','e','b','b','o','g','g','l','e','s' };
int rddTextHashAttinyArcade[13] = { '#','A','t','t','i','n','y','A','r','c','a','d','e' };
int rddTextGameOver[9] = { 'G','a','m','e',' ','O','v','e','r' };
int rddTextScoreLabel[6] = { 's','c','o','r','e',':' };
int rddTextTopScoreLabel[10] = { 't','o','p',' ','s','c','o','r','e',':' };
...
int rddComputeByte( int x, int page )
{
    if( rddState == RDD_STATE_INTRO )
    {
        ...
        {
            if( page == 4 ) return rddTextByteAt( rddTextWebbogglesCom, 14, 18, x );
            if( page == 6 ) return rddTextByteAt( rddTextTweetWebboggles, 17, 12, x );
            if( page == 7 ) return rddTextByteAt( rddTextHashAttinyArcade, 13, 22, x );
            return 0;
        }
    ...
    {
        if( page == 3 ) return rddTextByteAt( rddTextGameOver, 9, 32, x );
        if( page == 5 )
        {
            int pixels = rddTextByteAt( rddTextScoreLabel, 6, 32, x );
            pixels = pixels | rddTextByteAt( rddScoreDigits, rddScoreDigitCount, 70, x );
            return pixels;
        }
        if( page == 6 )
        {
            int pixels = rddTextByteAt( rddTextTopScoreLabel, 10, 32, x );
...
void gameRunDudeRun_init()
{
    ...
    // set), not a large positive one; this project's own wider int
