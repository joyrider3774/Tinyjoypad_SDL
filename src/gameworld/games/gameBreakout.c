...
// this project's own standalone UFO/Stacker entries - see `gameUFO.c` in
// this same directory). Matching that precedent, this is ported as its own
// standalone menu entry - the file's own `playUFO()` half was independently
// re-verified against the already-shipped UFO (a 34-line, all-whitespace/
// debounce-only diff confirms it's the same game, not a separate port) and
// is not touched here.
//
...
//   upstream quirk since nothing else depends on it), but this port's own
//   `avrCompat.h`-aliased `int`s don't truncate - masked with `&0xFF` at
//   the exact site, the same byte-truncation fix class as `md_drawColumn()`'s
//   own `value &= 0xFF` masking (see this repo's own CLAUDE.md, "Rendering
//   model").
...
//   step) - this port's own single-voice audio channel has no queue, so a
//   same-tick double call would only ever be audible as the last one. Left
//   as-is rather than built into a full sequencer: unlike this project's
//   other found sound-burst bugs (computed sweeps firing tens to hundreds
//   of calls every single tick), this is a rare, at-most-two-calls edge
//   case, not a systematic multi-call burst - a deliberate, considered
...
//   the "hold fire ~2s to mute/unmute" gesture is
...
// char*, not int*: Vircon32 string literals are int[] (one word per
// character) - a standard C literal is char[], so keeping this int* would
// silently reinterpret its bytes 4-at-a-time as garbage ints.
int brkTextByte( int x, int y, int startX, int pageY, char* str )
{
    if( y != pageY ) return 0;
