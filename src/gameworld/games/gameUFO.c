...
//   2-byte big-endian score at address 0/1, matching upstream exactly;
//   note this game's own live score can go negative, per the clamps
//   documented below, but `ufoTop` itself can never actually be written
//   negative, since a negative live score can never exceed its own
//   starting value of 0).
...
int ufoTextByteLen( int x, int y, int startX, int pageY, char* str, int strLen )
{
    if( y != pageY ) return 0;
...
void ufoRenderFrame( int mode )
{
    ...
    for( y = 0; y < 8; y++ )
    {
        // char*, not int*: Vircon32 string literals are int[] (one word
        // per character) - a standard C literal is char[], so keeping
        // this int* would silently reinterpret its bytes 4-at-a-time as
        // garbage ints. ufoTextByteLen() was switched to char* too.
        char* attractStr = "";
        int attractStartX = 0;
        int attractStrLen = 0;
