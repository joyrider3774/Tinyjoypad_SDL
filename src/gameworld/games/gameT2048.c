...
// tinyjoypad_vircon32 build's own gameT2048.c - converting Vircon32's own
// C dialect back to standard C. Dialect fixes applied in THIS conversion
// pass (Vircon32 -> standard C):
//  - `int[N] name` / `int[R][C] name` array declarations -> `int name[N]` /
//    `int name[R][C]` (standard C array-declarator order). This also
//    applies to the struct-typed board (`T2048Tile[SIZE][SIZE] t2048Board`
//    -> `T2048Tile t2048Board[SIZE][SIZE]`) and to the array-of-pointers
//    sound table (`int*[18] t2048SoundMergeTable` -> `int* t2048SoundMergeTable[18]`).
//  - `struct T2048Tile {...};` (Vircon32 allows a bare struct tag as a type
//    name) -> `typedef struct {...} T2048Tile;`.
//  - t2048LabelScore/Time/Game/Over (UI text passed to setString()) and
//    t2048ScoreString/t2048TimeString (built at runtime, also passed to
//    setString()) were `int[]` holding character data upstream -> `char[]`,
//    since setString()'s own signature is now `char*` (see obonoCoreShim.h's
//    own note: Vircon32 strings are `int[]`, standard C strings are
//    `char[]`). t2048Num2str()'s own `int* p` parameter was changed to
//    `char* p` to match, since its only two callers both pass one of these
//    text buffers. Every OTHER `int[]` table in this file (t2048ImgTile,
//    t2048ImgLogo, t2048ImgTitle, every t2048Sound* table) is genuine
//    numeric bitmap/sound data, not text, and stays `int`.
...
int t2048SoundStart[11] = { 72, 12, 74, 12, 76, 12, 77, 12, 79, 36, 0xFF };
int t2048SoundOver[17]  = { 55, 13, 54, 16, 53, 19, 52, 22, 51, 25, 50, 28, 49, 31, 48, 34, 0xFF };
int t2048SoundMove[3]  = { 59, 1, 0xFF };
int t2048SoundMerge4[5]  = { 74, 2, 78, 2, 0xFF };
int t2048SoundMerge8[7]  = { 77, 2, 81, 2, 85, 2, 0xFF };
int t2048SoundMerge16[9] = { 82, 2, 87, 2, 92, 2, 97, 2, 0xFF };
int t2048SoundMerge32[19] = { 69, 3, 81, 3, 93, 3, 72, 3, 84, 3, 96, 3, 77, 3, 89, 3, 101, 3, 0xFF };
int t2048SoundMerge64[19] = { 71, 4, 83, 4, 95, 4, 74, 4, 86, 4, 98, 4, 79, 4, 91, 4, 103, 4, 0xFF };
int t2048SoundMerge128[25] = { 72, 4, 84, 4, 96, 4, 76, 4, 88, 4, 100, 4, 79, 4, 91, 4, 103, 4, 84, 4, 96, 4, 108, 4, 0xFF };
int t2048SoundMerge256[17] = { 67, 6, 72, 6, 69, 6, 74, 6, 71, 6, 76, 6, 72, 6, 77, 6, 0xFF };
int t2048SoundMerge512[17] = { 69, 7, 73, 7, 76, 7, 81, 7, 71, 7, 74, 7, 78, 7, 83, 7, 0xFF };
int t2048SoundMerge1024[17] = { 71, 8, 75, 8, 78, 8, 77, 8, 75, 8, 77, 8, 78, 8, 83, 8, 0xFF };
int t2048SoundMerge2048[19] = { 72, 10, 79, 10, 76, 10, 79, 10, 81, 10, 77, 10, 83, 10, 79, 10, 84, 15, 0xFF };
...
char t2048LabelScore[6] = "SCORE";
char t2048LabelTime[5]  = "TIME";
char t2048LabelGame[5]  = "GAME";
char t2048LabelOver[5]  = "OVER";
...
int t2048State;
T2048Tile t2048Board[T2048_BOARD_SIZE][T2048_BOARD_SIZE];
int t2048Score;
int t2048Empty, t2048TimeMinutes, t2048TimeSeconds, t2048TimeFrames;
int t2048MoveVx, t2048MoveVy, t2048AddedIdx;
char t2048ScoreString[7];
char t2048TimeString[7];

int t2048Num2str( char* p, int pOffset, int value )
{
    while( true )
    {
