...
//   trick with no benefit on this project's own desktop target. More
//   importantly, tracing the exact shift arithmetic for row 23 (the top
//   spawn-buffer row - `y-15` reaches 8, a shift amount that only stays
//   safe on AVR because a `uint8_t & (1<<8)` is *always* 0 by definition,
//   and because AVR-GCC's own variable-shift-count codegen saturates to 0
//   past the operand width - both of them AVR-specific narrow-type
//   behaviors already documented elsewhere in this project as *not*
//   safely portable) revealed that a literal port of this bit-packing
//   would either need yet another shift-safety workaround for no real
//   benefit, or would quietly behave differently (and worse - shifting by
//   a genuinely negative amount, not just a wrapping one) on this
//   project's own plain, non-truncating `int`s (see avrCompat.h). Ported
//   instead as a plain `bool[10][24]` grid with the identical read/write
//   *interface* - correct by construction, avoiding the whole
//   shift-arithmetic hazard rather than working around it, unlike Tiny
//   Tris's own bit-packed grid (kept faithful there since no such hazard
//   was ever found in that game's own equivalent code).
...
//   to work around noisy analog voltage-divider buttons, which this
//   project's own clean digital gamepad reads don't need - matching this
//   project's own established simplification for every other `gametiny`
//   port's real-hardware debounce code) - ported instead as plain
//   edge-detected presses for move-left/right/rotate, and a level-held
//   check for soft-drop (drops once per tick while Down is held,
//   approximating upstream's own `delay(10)`-paced soft-drop busy-wait
//   loop closely enough at this engine's own 60fps tick rate).
...
//   (not once per row within that event) - this project's own audio
//   channel has no queue, so multiple same-tick sequencer restarts would
//   only ever play the last one's notes anyway, making the distinction
//   largely inaudible in practice; simplified rather than engineered
//   around for a difference this narrow. The game-over sweep
...
//   safety risk here (not just a wrong-looking glyph), so
...
int tetBlocks[7] = { 17476, 17600, 17504, 1632, 1728, 3648, 3168, };

int tetBlockout[16] = { 248, 0, 62, 128, 15, 224, 3, 248, 62, 128, 15, 224, 3, 248, 62, 0, };
int tetGhostout[16]  = { 136, 0, 34, 128, 8, 32, 2, 136, 34, 128, 8, 32, 2, 136, 34, 0, };

int tetBrickLogo[288] =
{
...
int tetCharByte( char* str, int strLen, int startPage, int startCol, int physPage, int physCol )
{
...
int tetHappyNotes[8] = { 55, 31, 105, 31, 155, 31, 205, 31, };
...
bool tetBlockGrid[10][24];
bool tetGhostGrid[10][24];

int tetNextBlockBuffer[8][2];
int tetNextPiece;
int tetScore;
int tetTop;
int tetLevel;
