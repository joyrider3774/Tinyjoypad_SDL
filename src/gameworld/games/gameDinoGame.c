...
//
// SDL3 port note: two call sites below passed raw string literals
// ("Game over :(", "Start") directly to a function expecting `int*`.
// That's valid in Vircon32's own dialect (string literals there are
// `int[]`, one word per character - see VIRCON32_C_DIALECT.md), but a
// standard C string literal is `char[]`, not assignment-compatible with
// `int*`. Converted those two literals into explicit file-scope `int`
// arrays of character codes (dinoTextGameOverStr/dinoTextStartStr,
// declared alongside the other text/font data below) - same technique
// this file already uses by hand for dinoScoreText, just applied to
// these two fixed strings as well. No other file/behavior change.
// =============================================================================
...
int dinoCactusImage[8] =
{
7,15,24,254,255,24,24,12,
};

// String literals used at the two dinoTextColByte() call sites in
// dinoComputeByte() below (see the SDL3 port note above) - explicit
// character-code arrays, exact length, no trailing terminator (the
// caller always passes an explicit textLen, so none is needed).
int dinoTextGameOverStr[12] = { 'G','a','m','e',' ','o','v','e','r',' ',':','(' };
int dinoTextStartStr[5] = { 'S','t','a','r','t' };
...
int dinoRockY[3];
int dinoRockWidth[3];
int dinoRockVelocity[3];
int dinoRockDelayTicks[3];
int dinoRockNextUpdateAt[3];

int dinoCactusX[6];        // fixed-point, DINO_NORM_VALUE-scaled
int dinoCactusSpriteX[6];  // real pixel x
int dinoCactusVelocity[6];
int dinoCactusNextAppearAt[6];
int dinoCactusStartDelay[6];
...
int dinoScoreText[24];
int dinoScoreTextLen;

int dinoCactusPageBuf[128];
...
int dinoComputeByte( int x, int page )
{
    if( dinoState == DINO_STATE_GAMEOVER )
    {
        ...
        if( ( page == 3 ) || ( page == 4 ) )
        {
            return dinoTextColByte( dinoTextGameOverStr, 12, 32, x, 25, page );
        }
        return 0;
    }
    ...
    if( dinoState == DINO_STATE_START )
    {
        ...
        if( ( page == 3 ) || ( page == 4 ) )
        {
            return dinoTextColByte( dinoTextStartStr, 5, 52, x, 25, page );
        }
        return 0;
    }
