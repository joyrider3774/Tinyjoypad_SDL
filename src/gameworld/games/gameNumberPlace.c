...
//
// This file is itself ported a second time, from the sibling
// tinyjoypad_vircon32 build's own gameNumberPlace.c (which already merged
// upstream's common.h/common.cpp/logo.cpp/title.cpp/game.cpp/data.h into
// one file and did the structural work below) - converting Vircon32's own
// C dialect back to standard C. Every file-scope name is prefixed np*/NP_*
// to avoid collisions with other games sharing this project's single
// game-world translation unit (see gamesMain.c).
//
// Dialect fixes applied in THIS conversion pass (Vircon32 -> standard C),
// beyond what the Vircon32 port itself already did to upstream:
//  - `int[N] name` / `int[R][C] name` array declarations -> `int name[N]` /
//    `int name[R][C]` (standard C array-declarator order).
//  - `struct NpCell {...}; NpCell board;` (Vircon32 allows a bare struct
//    tag as a type name) -> `typedef struct {...} NpCell;` (in obonoCoreShim.h
//    lineage this struct is local to this file, so it's typedef'd here).
//  - `npLevelString` was `int[28]` holding character literals (used as a
//    C-string via setString()) -> `char npLevelString[28]`, since
//    setString()'s own signature is now `char*` (see obonoCoreShim.h's own
//    note on why: Vircon32 strings are `int[]`, standard C strings are
//    `char[]`). Every OTHER `int[]` table in this file (npImgDigit,
//    npSoundStart, npPuzzleData, etc) is genuine numeric bitmap/sound/
//    puzzle data, not text, and stays `int`.
//  - `DrawFunc* drawFn` -> `DrawFunc drawFn` (obonoCoreShim.h's DrawFunc is
//    now a real function-pointer typedef that already includes the `*` -
//    see that header's own note).
//
// Structural changes already made by the Vircon32 port (kept as-is here -
// these aren't Vircon32-dialect-specific, they're genuine upstream
// restructuring that's still correct/needed on any platform):
//  - NP_FPS matches upstream's own real rate (20, not this engine's native
//    60fps) - gameNumberPlace_update() only actually ticks once every
//    NP_TICK_DIVISOR (3) real frames, so every frame-counted duration
//    (press-and-hold thresholds, dpad repeat delay) still measures the
//    same real-world time it did upstream.
...
// avrCompat.h/obonoCoreShim.h/machineDependent.h are already included
// earlier by gamesMain.c before this file (single translation unit via
// #include, same shape as the sibling Vircon32 build's own main.c).

// Upstream (TinyJoypadWorks/numberplace/common.h) ran its own fixed-
// timestep loop at a real FPS=20 - this port reproduces that by only
// actually ticking its own logic once every NP_TICK_DIVISOR real engine
...
// Arduino-style random()/randomSeed() built on this platform's real
// rand()/srand() (via avrCompat.h's arand()) - only the single-argument
// random(n) form is used anywhere in this port.
int npRandom( int n )
{
    return arand( n );
}
...
int npImgCursor[ 8 ] = { 0xff, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81, 0xff };
...
int npImgLocked[ 8 ] = { 0xF8, 0xFE, 0xF9, 0x99, 0xD9, 0xF9, 0xFE, 0xF8 };

#define NP_IMG_SELECTING_W 20
#define NP_IMG_SELECTING_H 8

int npImgSelecting[ 20 ] =
{
    0x40, 0xe0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xe0, 0x40
};

int npSoundStart[ 11 ]  = { 72, 12, 74, 12, 76, 12, 77, 12, 79, 36, 0xFF };
int npSoundPlace[ 11 ]  = { 90, 1, 91, 1, 92, 1, 93, 1, 94, 1, 0xFF };
int npSoundRemove[ 11 ] = { 90, 1, 89, 1, 88, 1, 87, 1, 86, 1, 0xFF };
int npSoundFailure[ 13 ] = { 87, 3, 75, 3, 63, 3, 87, 3, 75, 3, 63, 3, 0xFF };
int npSoundComplete[ 19 ] =
{
...
// directly - identical layout to upstream's levelString[]. This is real
// text (fed to setString()), unlike every other table on this page, so
// it's `char` rather than `int` - see this file's own header comment.
char npLevelString[ 28 ] =
{
    ' ','E','A','S','Y',' ',0,
    'M','E','D','I','U','M',0,
    ' ','H','A','R','D',' ',0,
...
int npTitleImgCursor[ 5 ] = { 0x38, 0x7C, 0x7C, 0x7C, 0x38 };
...
NpCell npBoard[ NP_BOARD_SIZE ][ NP_BOARD_SIZE ];
int npBoardState[ NP_SEGMENT_MAX ][ NP_BOARD_SIZE ];
int npBoardFailure;
...
void npInitPuzzle()
{
    ...
    int table[ NP_SEGMENT_MAX ][ NP_BOARD_SIZE ];
