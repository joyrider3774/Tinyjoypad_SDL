...
//    port here.
//  - EEPROM-backed high-score persistence restored (see eepromShim.h/.c) -
//    ported as a single level/lines/score triple rather than upstream's
//    own 4-slot checksummed backup scheme, since that redundancy exists
//    purely to guard against real AVR EEPROM wear/corruption, a concern
//    this port's own file-backed shim doesn't share (it already checksums
//    the whole slot on every load) - see trisRecupeHighscore()/
...
// divisor of its own (ships at full real 60fps rate), so wait-frame
// counts are computed against 60.
// -----------------------------------------------------------------------------

#define TRIS_SND_MAX_NOTES 4
int trisSndFreqBytes[TRIS_SND_MAX_NOTES];
int trisSndDurBytes[TRIS_SND_MAX_NOTES];
int trisSndLen;
int trisSndPos;
...
// AVR EEPROM wear/corruption - a concern this shim's own file-backed
// storage doesn't have (the shim already checksums the whole slot on
